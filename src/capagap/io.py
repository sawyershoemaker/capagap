...
def _normalize_address(address: Any) -> AddressRecord:
    ...
    kind = str(address.get("type", "unknown"))
    ...
    if value is None:
        display = "global" if raw_value is None else f"{kind}:{raw_value}"
...
def _normalize_rule(name: str, payload: Any) -> RuleRecord:
...
def load_document(
    path: str | Path, *, expected_flavor: str | None = None
) -> CapaDocument:
    ...
    rules = {
        str(name): _normalize_rule(str(name), value)
